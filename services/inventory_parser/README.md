...
| `cache/icon_embeddings_{type}_labels.json` / `_rarities.json` / `_shapes.json` | Per-embedding metadata — item ID, rarity, and shape (circularity/aspect ratio) keyed by row index — built alongside the `.npy` embeddings |
